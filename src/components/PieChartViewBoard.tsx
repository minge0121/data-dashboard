...
// 饼图（环形图）
export const PieChartViewBoard = () => {
  ...
  const option: EChartsOption = {
    ...
    series: [
      {
        type: "pie", // 饼图（环形图）
        radius: ["42%", "68%"], // 内径42%，外径68% → 环形 。内外径不同形成环形（类似甜甜圈图）。如果只有一个值则变成实心饼图。
        center: ["35%", "50%"], // 圆心位置：水平方向距左侧 35%，垂直方向 50% 处。偏左侧，留出右侧空间给图例。
        ...
        data: pieChartData, // 外部传入的数据数组，通常格式为 [{ name: "A", value: 100 }, ...]
      },
    ],
